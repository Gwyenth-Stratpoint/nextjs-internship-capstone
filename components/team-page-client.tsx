...
import { ArrowUpRight, Calendar, Clock3, Mail, MoreHorizontal, Sparkles, UserPlus } from "lucide-react";
import { useMemo, useState } from "react";

import { CardBadge } from "@/components/cards/card-primitives";
...
function TeamCardShell({
  ...
  return (
    <article
      className={`relative overflow-hidden rounded-[20px] border border-white/70 bg-white shadow-sm ${className}`}
    >
      <div className={`absolute inset-x-0 top-0 h-[3px] ${accentClassName}`} />
      {children}
    </article>
  );
}
  ...
  return (
    <>
      <div className="space-y-5">
        <section className="glass-shell rounded-[24px] px-5 py-4">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <div className="rounded-[18px] border border-white/70 bg-white/80 px-4 py-3 shadow-sm">
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-slate-500">Total members</p>
              <p className="mt-1 text-[1.45rem] font-semibold text-slate-900">{summary.members}</p>
            </div>
            <div className="rounded-[18px] border border-white/70 bg-white/80 px-4 py-3 shadow-sm">
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-slate-500">Active now</p>
              <p className="mt-1 text-[1.45rem] font-semibold text-slate-900">{summary.activeNow}</p>
            </div>
            <div className="rounded-[18px] border border-white/70 bg-white/80 px-4 py-3 shadow-sm">
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-slate-500">Pending invites</p>
              <p className="mt-1 text-[1.45rem] font-semibold text-slate-900">{summary.invites}</p>
            </div>
          </div>
        </section>

        <div className="grid gap-5 xl:grid-cols-[1.7fr,0.78fr]">
          <section className="glass-shell rounded-[24px] p-5">
            <div className="border-b border-white/60 pb-4">
              <div>
                <h2 className="text-[1.2rem] font-semibold text-slate-900">Team members</h2>
                <p className="mt-1 text-sm text-slate-500">
                  {snapshot.members.length} member{snapshot.members.length === 1 ? "" : "s"} in this workspace
                </p>
              </div>
            </div>

            {snapshot.members.length === 0 ? (
              <div className="mt-4 rounded-[18px] border border-dashed border-white/70 bg-white/60 px-4 py-8 text-sm text-slate-500">
                No team members yet.
              </div>
            ) : (
              <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3">
                {snapshot.members.map((member) => (
                  <TeamCardShell
                    key={member.id}
                    className="glass-card rounded-2xl p-4 transition-colors hover:bg-white/72"
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3 text-sm">
                      <span className={`rounded-xl border px-3 py-1.5 text-sm font-medium capitalize ${getRoleTone(member.role)}`}>
          ...
          <div className="space-y-5">
            <section className="glass-shell rounded-[24px] p-5">
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {snapshot.invitations.length} invitation{snapshot.invitations.length === 1 ? "" : "s"} waiting for response
                </p>
              </div>

              {snapshot.invitations.length === 0 ? (
                <div className="rounded-[18px] border border-dashed border-white/70 bg-white/60 px-4 py-8 text-sm text-slate-500">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-base font-semibold text-slate-900">{invite.email}</p>
                          <p className="mt-1 text-sm text-slate-500">Invited as {formatRole(invite.role)}</p>
            ...
            <section className="glass-shell rounded-[24px] border border-blue-200/70 bg-blue-50/80 p-4">
              <p className="text-base font-semibold text-blue-900">Role changes and permissions</p>
              <p className="mt-2 text-sm leading-6 text-blue-800">
                Open workspace settings when you need to adjust organization roles or review invitation access.
              </p>
            </section>
          </div>
        </div>
