...
function getInitialTaskState(task?: TaskDraft | null): TaskDraft {
  if (!task) {
    return emptyTask;
  }
...
const taskModalSchema = taskSchema
  ...
  .extend({
    ...
    labels: z
      .array(z.string().trim().min(1, "Label cannot be empty").max(24, "Label is too long"))
      .max(8, "Add up to 8 labels"),
  });
...
function SummaryCard({
  ...
  const selectedAssignee =
    assigneeOptions.find((assignee) => assignee.id === form.assigneeId) ?? null;
    ...
    <div className="rounded-[20px] border border-slate-200 bg-white">
      <div className="grid grid-cols-[108px_minmax(0,1fr)] items-start gap-x-3 gap-y-3 px-3.5 py-3.5 sm:grid-cols-[120px_minmax(0,1fr)]">
        <ModalDetailRow icon={<CircleDashed size={16} />} label="Status">
          ...
          <TaskChip tone={getStatusChipClass(form.status)}>
        ...
        <ModalDetailRow icon={<UserRound size={16} />} label="Assignee">
          {isEditing ? (
            <select
              ...
              onChange={(event) =>
                setForm((current) => ({ ...current, assigneeId: event.target.value || null }))
              }
              className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
            >
          ...
          <span className="text-sm font-medium text-slate-700">
            {selectedAssignee ? selectedAssignee.name : "No assignee yet"}
          </span>
        </ModalDetailRow>
            ...
            {isEditing ? (
              <>
                ...
                <p className="text-xs text-slate-500">
                  Using a textarea for now. You can swap this to a rich text editor later.
                </p>
              </>
            ) : (
          ...
          <div className="flex flex-wrap gap-2">
            {form.labels.length > 0 ? (
              form.labels.map((label) => (
                <span
                  key={label}
                  className="inline-flex items-center gap-2 rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700"
                >
                  {label}
                  <button
                    type="button"
...
export function CreateTaskModal({
  ...
  async function handleDelete() {
    ...
    const confirmed = window.confirm(
      `Delete "${form.title || "this task"}"? This cannot be undone.`,
    );
    if (!confirmed) return;
      ...
      <SideSheetSurface>
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 space-y-3.5 overflow-y-auto px-4 py-4">
            ...
            <div
              className={`grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_10rem] ${!isEditing ? "pointer-events-none opacity-80" : ""}`}
            >
              <div className="space-y-1.5">
            ...
            {isEditing ? (
              ...
              <div className="flex items-center justify-between border-t border-slate-200 pt-0">
                <p className="max-w-[240px] text-xs text-slate-500">
                  Click edit to make changes to this task.
                </p>
                <button
                  type="button"
                  onClick={onClose}
